...
class ComportementAnalyzer {
    ...
    public static function get_real_time_stats() {
        ...
        $stats['recent_page_views'] = $wpdb->get_var("
            SELECT COUNT(*)
            FROM $table_events 
        ...
        $stats['recent_product_views'] = $wpdb->get_var("
            SELECT COUNT(*)
            FROM $table_events 
        ...
        $stats['recent_cart_adds'] = $wpdb->get_var("
            SELECT COUNT(*)
            FROM $table_events 
        ...
        $stats['top_current_pages'] = $wpdb->get_results("
            ...
            WHERE event_type = 'page_view'
            AND timestamp >= DATE_SUB(NOW(), INTERVAL 1 HOUR)
            GROUP BY JSON_EXTRACT(event_data, '$.page_url')
            ORDER BY views DESC
            LIMIT 10
        ");
