...
const elems = document.querySelectorAll('.nav-item-signinup');
elems.forEach(bout => {
  bout.addEventListener('click', els => {
    elems.forEach(e => {
      e.classList.remove('active');
      e.querySelector('a').classList.remove('active');
    });

    els.target.parentNode.classList.add('active');
    els.target.classList.add('active');

    if (els.target.classList.contains('tab-connexion')) {
