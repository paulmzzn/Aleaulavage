...
add_action('wp_enqueue_scripts', function() {
    ...
    if (is_front_page()) {
        wp_enqueue_script(
            'lucide-icons',
            get_stylesheet_directory_uri() . '/js/lucide.min.js',
            array(),
            $theme_version ?? '1.0',
            true // Charger en bas de page pour de meilleures performances
        );
...
function daz_scripts_loader() {
	...
	// 1. Styles avec preload pour les critiques.
	wp_enqueue_style( 'style', get_theme_file_uri( 'style.css' ), array(), $theme_version, 'all' );
	wp_enqueue_style( 'main', get_theme_file_uri( 'css/main.min.css' ), array(), $theme_version, 'all' ); // main.scss: Compiled Framework source + custom styles.
	
	// FontAwesome seulement si nécessaire
...
require_once get_stylesheet_directory() . '/includes/admin-comportement-nouveau.php';
...
// =================
// QUERY MONITOR - FORCER L'AFFICHAGE
// =================

// Forcer l'affichage de Query Monitor dans l'admin bar
add_action('plugins_loaded', function() {
    if (current_user_can('manage_options')) {
        // S'assurer que l'admin bar est visible
        add_filter('show_admin_bar', '__return_true');
        
        // Forcer Query Monitor à s'afficher même en production
        add_filter('qm/show_query_monitor', '__return_true');
        
        // Désactiver les conditions qui cachent QM
        add_filter('qm/disable', '__return_false');
        
        // Forcer l'affichage pour les admins
        if (class_exists('QM')) {
            add_filter('qm/hide_qm', '__return_false');
        }
    }
}, 1);

// S'assurer que WP_DEBUG est activé pour Query Monitor
if (!defined('WP_DEBUG')) {
    define('WP_DEBUG', true);
}

// Debug pour vérifier si Query Monitor est chargé
add_action('wp_footer', function() {
    if (current_user_can('manage_options')) {
        echo '<!-- QM Debug: ';
        echo class_exists('QueryMonitor') ? 'QM-LOADED' : 'QM-NOT-LOADED';
        echo ' | User: ' . wp_get_current_user()->user_login;
        echo ' | Admin Bar: ' . (is_admin_bar_showing() ? 'VISIBLE' : 'HIDDEN');
        echo ' -->';
    }
});

// Forcer l'activation du plugin Query Monitor si il existe
...
// 2. Cache pour les requêtes lourdes
function cache_expensive_query($cache_key, $callback, $expiration = HOUR_IN_SECONDS) {
    $cached_result = wp_cache_get($cache_key, 'expensive_queries');
    ...
    wp_cache_set($cache_key, $result, 'expensive_queries', $expiration);
    
    return $result;
}

// 3. Optimiser WooCommerce
add_action('init', function() {
    // Désactiver les scripts WC non nécessaires
...
add_action('wp_head', function() {
    if (is_front_page()) {
        echo '<link rel="preload" href="' . get_theme_file_uri('css/main.min.css') . '" as="style" onload="this.onload=null;this.rel=\'stylesheet\'">';
        echo '<link rel="preload" href="' . get_theme_file_uri('assets/fontawesome/css/all.min.css') . '" as="style" onload="this.onload=null;this.rel=\'stylesheet\'">';
    }
}, 1);

// 5. Compression gzip
add_action('init', function() {
    if (!ob_get_level() && !wp_doing_ajax() && !wp_doing_cron()) {
        ob_start('ob_gzhandler');
    }
});

// 6. Headers de cache pour les assets
